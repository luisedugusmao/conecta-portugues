...
// --- NOVO COMPONENTE DE FUNDO ANIMADO (ILUMINAÇÃO INTENSIFICADA & GRADE REATIVA) ---
const AnimatedGridBackground = ({ children }) => {
  ...
  return (
    // Fundo claro da paleta (#f4efea) com listener de mouse
    <div
      className="relative w-full min-h-screen bg-[#f4efea] overflow-hidden flex flex-col items-center justify-center p-4 group"
      onMouseMove={handleMouseMove}
    >

      {/* 1. Grade Base (Estática e Suave) */}
      ...
      {/* 2. Grade Interativa (Brilhante e Revelada pelo Mouse) */}
      {/* Esta camada é mais forte (#a51a8f80) e usa mask-image para aparecer apenas perto do cursor */}
      <div
        className="absolute inset-0 bg-[linear-gradient(to_right,#a51a8f80_1px,transparent_1px),linear-gradient(to_bottom,#a51a8f80_1px,transparent_1px)] bg-[size:40px_40px] pointer-events-none transition-opacity duration-300"
        style={{
      ...
      {/* 3. Luz Ambiente Dourada (Mais intensa) */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background: `radial-gradient(600px circle at ${mousePosition.x}px ${mousePosition.y}px, rgba(238, 192, 10, 0.15), transparent 40%)`
        }}
      ></div>

      {/* 4. Luz Central Roxa (Foco intenso no cursor) */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background: `radial-gradient(300px circle at ${mousePosition.x}px ${mousePosition.y}px, rgba(165, 26, 143, 0.15), transparent 30%)`
        }}
      ></div>
...
const NavButton = ({ active, onClick, icon, label, dark = false }) => (
  <button
    ...
    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${active
      ...
      : dark ? 'text-white/60 hover:bg-white/10 hover:text-white' : 'text-slate-500 hover:bg-slate-100'
      }`}
  >
...
const ViewHome = ({ student, classes }) => {
            ...
            return (
              <div key={cls.id} className={`relative pl-12 md:pl-20 transition-all duration-500 ${isLive ? 'z-20' : isSoon ? 'z-10' : 'opacity-90 hover:opacity-100'}`}>
                ...
                <div className={`rounded-2xl border transition-all duration-300 relative ${isLive ? 'bg-white border-[#a51a8f] shadow-2xl shadow-[#a51a8f]/20 p-6 md:p-8 scale-105 origin-left ring-2 ring-[#a51a8f]/10' : isSoon ? 'bg-[#fff9db] border-[#eec00a] p-6 shadow-md scale-100' : 'bg-slate-50 border-slate-200 p-5 hover:bg-white hover:shadow-md'}`}>
                  <div className="flex justify-between items-start mb-3"><span className={`text-xs font-bold uppercase tracking-wider ${isLive ? 'text-[#a51a8f]' : isSoon ? 'text-yellow-700' : 'text-slate-400'}`}>{cls.date}</span>{isLive && <span className="bg-red-500 text-white text-xs px-3 py-1 rounded-full font-extrabold shadow-sm animate-pulse">AO VIVO</span>}{isSoon && <span className="bg-[#eec00a] text-[#7d126b] text-xs px-3 py-1 rounded-full font-bold">EM BREVE</span>}{isCompleted && <span className="bg-green-100 text-green-700 text-xs px-2 py-1 rounded font-bold">Encerrada</span>}{isLocked && <span className="bg-slate-200 text-slate-500 text-xs px-2 py-1 rounded font-bold">Bloqueada</span>}</div>
                  <h3 className={`font-bold text-slate-800 mb-2 ${isLive ? 'text-2xl' : 'text-lg'}`}>{cls.title}</h3>
                  <p className={`mb-4 ${isLive ? 'text-slate-600 text-base' : 'text-slate-500 text-sm'}`}>{cls.description}</p>
                  {!isLocked && (
                    <div className={`flex flex-wrap gap-3 pt-4 border-t ${isLive ? 'border-[#a51a8f]/10' : isSoon ? 'border-yellow-200' : 'border-slate-200'}`}>
                      {cls.recordingLink ? (<a href={cls.recordingLink} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-bold bg-slate-200 text-slate-700 hover:bg-slate-300 transition-colors"><PlayCircle className="w-4 h-4" />Assistir Gravação</a>) : (cls.meetLink && (<a href={cls.meetLink} target="_blank" rel="noreferrer" className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-bold transition-colors ${isLive ? 'bg-[#a51a8f] text-white hover:bg-[#7d126b] shadow-md shadow-[#a51a8f]/30' : isSoon ? 'bg-[#eec00a] text-[#7d126b] hover:bg-[#d4ab09]' : 'bg-slate-200 text-slate-700 hover:bg-slate-300'}`}><Video className="w-4 h-4" />{isLive ? 'Entrar na Aula' : isSoon ? 'Link da Aula' : 'Acessar'}</a>))}
                      {cls.materials?.map((mat, i) => (<button key={i} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isLive || isSoon ? 'bg-white/50 text-slate-700 hover:bg-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}><FileText className="w-4 h-4" />{mat.title}</button>))}
                    </div>
                  )}
                </div>
              </div>
...
const ViewCalendar = ({ classes }) => {
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-0 animate-fadeIn relative">
      <header className="flex justify-between items-center"><div><h2 className="text-2xl font-bold text-slate-800">Calendário de Aulas</h2><p className="text-slate-500">Organize-se com a programação mensal</p></div><div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-slate-200 shadow-sm"><ChevronLeft className="w-5 h-5 text-slate-400" /><span className="font-bold text-slate-700">Outubro</span><ChevronRight className="w-5 h-5 text-slate-400" /></div></header>
      <div className="bg-white rounded-3xl shadow-lg border border-slate-200 overflow-hidden p-6">
        ...
        <div className="grid grid-cols-7 gap-2 md:gap-4">{Array.from({ length: startDayOffset }).map((_, i) => (<div key={`empty-${i}`} className="aspect-square"></div>))}{days.map(day => { const hasClass = classesByDay[day]?.length > 0; const isSelected = selectedDay === day; const status = hasClass ? classesByDay[day][0].status : null; let bgClass = 'bg-slate-50 hover:bg-slate-100 text-slate-700'; if (isSelected) bgClass = 'bg-[#a51a8f] text-white shadow-lg ring-4 ring-[#fdf2fa]'; else if (status === 'live') bgClass = 'bg-[#fdf2fa] border-2 border-[#a51a8f] text-[#a51a8f]'; else if (status === 'soon') bgClass = 'bg-[#fff9db] border-2 border-[#eec00a] text-[#7d126b]'; else if (hasClass) bgClass = 'bg-slate-100 border-2 border-slate-200 text-slate-600'; return (<button key={day} onClick={() => setSelectedDay(day)} className={`aspect-square rounded-2xl flex flex-col items-center justify-center relative transition-all duration-200 ${bgClass}`}><span className={`text-lg md:text-xl font-bold ${isSelected ? 'scale-110' : ''}`}>{day}</span>{hasClass && (<div className={`w-1.5 h-1.5 rounded-full mt-1 ${isSelected ? 'bg-white' : status === 'live' ? 'bg-[#a51a8f] animate-pulse' : 'bg-slate-400'}`}></div>)}</button>); })}</div>
      </div>
      {selectedDay && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fadeIn" onClick={() => setSelectedDay(null)}>
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-slideUp" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 max-h-[60vh] overflow-y-auto">{selectedClasses && selectedClasses.length > 0 ? (<div className="space-y-4">{selectedClasses.map(cls => (<div key={cls.id} className="flex items-start gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100"><div className={`p-3 rounded-lg ${cls.status === 'live' ? 'bg-[#fdf2fa] text-[#a51a8f]' : 'bg-slate-200 text-slate-500'}`}>{cls.status === 'live' ? <Radio className="w-6 h-6 animate-pulse" /> : <Video className="w-6 h-6" />}</div><div><h4 className="font-bold text-slate-800">{cls.title}</h4><p className="text-sm text-slate-500 mb-2">{cls.description}</p><div className="flex gap-2"><span className={`text-xs font-bold px-2 py-1 rounded ${cls.status === 'live' ? 'bg-red-100 text-red-600' : 'bg-slate-200 text-slate-600'}`}>{cls.status === 'live' ? 'AO VIVO' : cls.status === 'completed' ? 'ENCERRADA' : 'AGENDADA'}</span></div></div></div>))}</div>) : (<div className="text-center py-8"><div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4 text-slate-400"><CalendarDays size={32} /></div><p className="text-slate-500 font-medium">Nenhuma aula registrada para este dia.</p></div>)}</div>
            <div className="p-4 border-t border-slate-100 bg-slate-50 text-center"><button onClick={() => setSelectedDay(null)} className="text-[#a51a8f] font-bold text-sm hover:underline">Fechar</button></div>
          </div>
        </div>
      )}
    </div>
...
const ViewChallenges = ({ student, quizzes, onCompleteQuiz }) => {
  ...
  if (activeQuiz) {
    ...
    if (isGradedView) {
      return (
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden min-h-[500px] flex flex-col animate-slideUp">
          ...
          <div className="p-6 flex-1 overflow-y-auto space-y-6">
            <div className="flex gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
              <div className="flex-1 text-center border-r border-slate-200">
                ...
                <p className="text-3xl font-bold text-[#a51a8f]">{sub.score} / {sub.totalQuestions}</p>
              </div>
              ...
              <div className="flex-1 text-center">
                ...
                <p className="text-sm font-bold text-slate-700">{sub.gradedAt ? new Date(sub.gradedAt.seconds * 1000).toLocaleDateString() : 'Recentemente'}</p>
              </div>
            </div>

            {sub.teacherFeedback && (
              <div className="bg-[#fdf2fa] p-4 rounded-xl border border-[#a51a8f]/20">
                <h4 className="font-bold text-[#a51a8f] mb-2 flex items-center gap-2"><MessageSquare size={16} /> Comentário do Professor</h4>
...
const ViewRank = ({ students, currentStudentId }) => {
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-0">
      <header className="text-center md:text-left"><h2 className="text-2xl font-bold text-slate-800">Ranking da Turma</h2><p className="text-slate-500">Quem será o Mestre da Língua?</p></header>
      <div className="bg-white rounded-3xl shadow-lg border border-slate-200 overflow-hidden"><div className="bg-[#a51a8f] p-4 text-white font-bold grid grid-cols-6 gap-2 text-sm md:text-base"><div className="col-span-1 text-center">#</div><div className="col-span-3">Aluno</div><div className="col-span-2 text-right">XP</div></div><div className="divide-y divide-slate-100">{sortedStudents.map((st, idx) => { const isMe = st.id === currentStudentId; let rankIcon = null; if (idx === 0) rankIcon = '🥇'; if (idx === 1) rankIcon = '🥈'; if (idx === 2) rankIcon = '🥉'; return (<div key={st.id} className={`grid grid-cols-6 gap-2 p-4 items-center ${isMe ? 'bg-[#fff9db]' : 'hover:bg-slate-50'}`}><div className="col-span-1 text-center font-bold text-slate-600 flex justify-center items-center">{rankIcon ? <span className="text-2xl">{rankIcon}</span> : `#${idx + 1}`}</div><div className="col-span-3 flex items-center gap-3"><div className="w-10 h-10 bg-[#fdf2fa] rounded-full flex items-center justify-center text-xl border border-[#a51a8f]/20 overflow-hidden">{st.photoUrl ? <img src={st.photoUrl} alt={st.name} className="w-full h-full object-cover" /> : st.avatar}</div><div className="flex flex-col"><span className={`font-bold ${isMe ? 'text-[#a51a8f]' : 'text-slate-700'}`}>{st.name} {isMe && '(Você)'}</span><span className="text-xs text-slate-400">Nível {st.level}</span></div></div><div className="col-span-2 text-right font-mono font-bold text-[#a51a8f]">{st.xp.toLocaleString()} XP</div></div>); })}</div></div>
    </div>
  );
};

// --- VIEW CORRECTIONS (TEACHER) ---
const ViewCorrections = ({ students }) => {
      ...
      <div className="bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col animate-slideUp">
        <div className="bg-[#2d1b36] p-6 text-white flex justify-between items-center">
          <div>
            ...
            <p className="text-white/60 text-sm">{selectedSubmission.studentName} - {selectedSubmission.quizTitle}</p>
          </div>
          ...
          submissions.map(sub => (
            <div key={sub.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="flex items-center gap-4 w-full md:w-auto">
                ...
                <div>
                  <h4 className="font-bold text-slate-800">{sub.quizTitle}</h4>
...
const AdminDashboard = ({ currentUser, students, classes, quizzes, onLogout }) => {
  ...
  const handleEditChallenge = (quiz) => { setNewChallenge({ id: quiz.id, title: quiz.title, xpReward: quiz.xpReward, coinReward: quiz.coinReward, questions: quiz.questions, assignedTo: quiz.assignedTo || [], deadline: quiz.deadline || '' }); setShowChallengeForm(true); window.scrollTo({ top: 0, behavior: 'smooth' }); };

  const saveChallenge = async () => {
    if (!newChallenge.title || newChallenge.questions.length === 0) return alert("Adicione título e questões.");
    const data = { title: newChallenge.title, xpReward: newChallenge.xpReward, coinReward: newChallenge.coinReward, questions: newChallenge.questions, assignedTo: newChallenge.assignedTo || [], deadline: newChallenge.deadline || '' };
...
const LoginWall = ({ onLogin }) => {
  ...
  const admins = students.filter(s => s.role === 'admin' || s.role === 'teacher');
  const regularStudents = students.filter(s => s.role === 'student');
  const displayedUsers = showAdminLogin ? admins : regularStudents;
  ...
  return (
    <AnimatedGridBackground>
      ...
      <div className="z-10 w-full max-w-4xl text-center flex flex-col items-center">
        ...
        <p className="text-slate-500 mb-12 text-lg font-medium">{showAdminLogin ? 'Área Restrita - Selecione o Usuário' : 'Selecione seu perfil para entrar'}</p>
        {!selectedStudent ? (
          <>
              ...
              {displayedUsers.map(student => (
                <button key={student.id} onClick={() => setSelectedStudent(student)} className="group flex flex-col items-center transition-all duration-300 transform hover:scale-105">
                  <div className={`w-24 h-24 md:w-32 md:h-32 rounded-3xl border-2 flex items-center justify-center text-5xl shadow-xl shadow-slate-200 group-hover:shadow-2xl relative overflow-hidden transition-all ${student.role !== 'student' ? 'bg-[#2d1b36] border-[#eec00a] text-white' : 'bg-white border-slate-100 group-hover:border-[#eec00a]'}`}>
                    ...
                    {student.role === 'student' && !student.photoUrl && <div className="absolute inset-0 bg-gradient-to-t from-[#a51a8f]/5 to-transparent"></div>}
                  </div>
                  <span className={`mt-4 text-lg font-bold group-hover:text-[#a51a8f] ${student.role !== 'student' ? 'text-[#2d1b36]' : 'text-slate-600'}`}>{student.name}</span>
                  {student.role !== 'student' && <span className="text-[10px] uppercase font-bold text-[#a51a8f] tracking-widest mt-1">{student.role === 'admin' ? 'Diretoria' : 'Professor'}</span>}
                </button>
              ))}
          ...
          <div className="bg-white/80 backdrop-blur-xl p-8 rounded-3xl shadow-2xl shadow-slate-200/50 max-w-sm mx-auto animate-fadeIn border border-white">
            <div className="mb-6 flex flex-col items-center"><div className={`w-20 h-20 rounded-full flex items-center justify-center text-4xl mb-3 ring-4 shadow-lg overflow-hidden ${selectedStudent.role !== 'student' ? 'bg-[#2d1b36] ring-[#eec00a]' : 'bg-[#fdf2fa] ring-white'}`}>{selectedStudent.photoUrl ? <img src={selectedStudent.photoUrl} alt={selectedStudent.name} className="w-full h-full object-cover" /> : selectedStudent.avatar}</div><h2 className="text-2xl font-bold text-slate-800">Olá, {selectedStudent.name}!</h2></div>
            <input type="password" value={password} onChange={(e) => { setPassword(e.target.value); setError(''); }} placeholder={selectedStudent.role !== 'student' ? "Senha de Acesso" : "Sua senha (1234)"} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 text-center text-lg tracking-widest focus:outline-none focus:border-[#a51a8f] focus:ring-2 focus:ring-[#a51a8f]/10 mb-4 transition-all" onKeyDown={(e) => e.key === 'Enter' && handleLogin()} />
            {error && <p className="text-red-500 text-sm mb-4 font-medium">{error}</p>}
            <div className="flex gap-2"><button onClick={() => { setSelectedStudent(null); setPassword(''); setError(''); }} className="flex-1 py-3 rounded-xl bg-slate-100 text-slate-600 hover:bg-slate-200 font-bold transition-colors">Voltar</button><button onClick={handleLogin} className="flex-1 py-3 rounded-xl bg-[#a51a8f] text-white hover:bg-[#8e167b] font-bold shadow-lg shadow-[#a51a8f]/30">Entrar</button></div>
          </div>
        )}
      </div>
    </AnimatedGridBackground>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 flex flex-col md:flex-row">
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-slate-200 h-screen sticky top-0"><div className="p-6 border-b border-slate-100 flex justify-center"><LogoSVG className="w-40 h-auto" /></div><nav className="flex-1 p-4 space-y-2"><NavButton active={currentView === 'home'} onClick={() => setCurrentView('home')} icon={<Home />} label="Início" /><NavButton active={currentView === 'journey'} onClick={() => setCurrentView('journey')} icon={<BookOpen />} label="Jornada" /><NavButton active={currentView === 'calendar'} onClick={() => setCurrentView('calendar')} icon={<CalendarDays />} label="Calendário" /><NavButton active={currentView === 'challenges'} onClick={() => setCurrentView('challenges')} icon={<Gamepad2 />} label="Desafios" /><NavButton active={currentView === 'rank'} onClick={() => setCurrentView('rank')} icon={<Trophy />} label="Ranking" /></nav><div className="p-4 border-t border-slate-100"><button onClick={handleLogout} className="flex items-center gap-3 w-full p-3 text-slate-500 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all font-medium text-sm"><LogOut size={18} /> Sair</button></div></aside>
      <main className="flex-1 max-w-5xl mx-auto w-full p-4 md:p-8"><header className="md:hidden flex justify-between items-center mb-6"><div className="w-32"><LogoSVG className="w-full h-auto" /></div><button onClick={handleLogout} className="p-2 bg-slate-200 rounded-full"><LogOut size={16} /></button></header><div className="animate-fadeIn">{currentView === 'home' && <ViewHome student={currentUser} classes={classes} />}{currentView === 'journey' && <ViewJourney classes={classes} />}{currentView === 'calendar' && <ViewCalendar classes={classes} />}{currentView === 'challenges' && <ViewChallenges student={currentUser} quizzes={quizzes} onCompleteQuiz={handleCompleteQuiz} />}{currentView === 'rank' && <ViewRank students={students} currentStudentId={currentUser.id} />}</div></main>
      <nav className="md:hidden fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 flex justify-around p-3 z-50 pb-safe"><MobileNavButton active={currentView === 'home'} onClick={() => setCurrentView('home')} icon={<Home size={20} />} label="Início" /><MobileNavButton active={currentView === 'journey'} onClick={() => setCurrentView('journey')} icon={<BookOpen size={20} />} label="Aulas" /><MobileNavButton active={currentView === 'calendar'} onClick={() => setCurrentView('calendar')} icon={<CalendarDays size={20} />} label="Agenda" /><MobileNavButton active={currentView === 'challenges'} onClick={() => setCurrentView('challenges')} icon={<Gamepad2 size={20} />} label="Jogar" /><MobileNavButton active={currentView === 'rank'} onClick={() => setCurrentView('rank')} icon={<Trophy size={20} />} label="Rank" /></nav>
    </div>
  );
}