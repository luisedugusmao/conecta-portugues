...
import { getClassStatus, isClassToday, formatDate } from '../utils/classHelpers';
        ...
        <div className="space-y-6 pb-20 md:pb-0">
            ...
            <div className="relative">
                ...
                <div className="space-y-8">
                    {classes.map((cls, idx) => {
                        const status = getClassStatus(cls); // Dynamic status
                        const isToday = isClassToday(cls.scheduledAt);

                        const isLive = status === 'live';
                        const isSoon = status === 'soon';
                        const isCompleted = status === 'completed';
                        const isLocked = cls.status === 'locked' && !cls.scheduledAt; // Fallback for legacy locked

                        // Highlight condition: Live OR (Today AND Not Completed)
                        const isHighlighted = isLive || (isToday && !isCompleted);

                        const displayDate = cls.scheduledAt ? formatDate(cls.scheduledAt) : cls.date;

                        return (
                            <div key={cls.id} className={`relative pl-12 md:pl-20 transition-all duration-500 ${isHighlighted ? 'z-20 scale-100' : 'opacity-90 hover:opacity-100'}`}>
                                ...
                                <div className={`rounded-2xl border transition-all duration-300 relative ${isLive ? 'bg-white dark:bg-slate-800 border-[#a51a8f] shadow-2xl shadow-[#a51a8f]/20 p-6 md:p-8 scale-105 origin-left ring-2 ring-[#a51a8f]/10' : isHighlighted ? 'bg-white dark:bg-slate-800 border-[#a51a8f] shadow-xl p-6 md:p-8 ring-1 ring-[#a51a8f]/20' : isSoon ? 'bg-[#fff9db] dark:bg-yellow-900/10 border-[#eec00a] p-6 shadow-md' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 p-5 hover:bg-white dark:hover:bg-slate-800 hover:shadow-md'}`}>
                                    <div className="flex justify-between items-start mb-3">
                                        <span className={`text-xs font-bold uppercase tracking-wider ${isLive ? 'text-[#a51a8f]' : isSoon ? 'text-yellow-700 dark:text-yellow-500' : 'text-slate-400'}`}>{displayDate}</span>
                                        {isLive && <span className="bg-red-500 text-white text-xs px-3 py-1 rounded-full font-extrabold shadow-sm animate-pulse">AO VIVO</span>}
                                        {isSoon && !isLive && <span className="bg-[#eec00a] text-[#7d126b] text-xs px-3 py-1 rounded-full font-bold">EM BREVE</span>}
                                        {isCompleted && <span className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-xs px-2 py-1 rounded font-bold">Encerrada</span>}
                                        {isLocked && <span className="bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400 text-xs px-2 py-1 rounded font-bold">Bloqueada</span>}
                                    </div>
                                    <h3 className={`font-bold text-slate-800 dark:text-white mb-2 ${isLive || isHighlighted ? 'text-2xl' : 'text-lg'}`}>{cls.title}</h3>
                                    <p className={`mb-4 ${isLive || isHighlighted ? 'text-slate-600 dark:text-slate-300 text-base' : 'text-slate-500 dark:text-slate-400 text-sm'}`}>{cls.description}</p>
                                    {!isLocked && (
